...
('4', 'Middle East and Africa\r');
 ...
 select departments.location_id, street_address, city, state_province, country_name, department_name from departments JOIN locations ON departments.location_id = locations.location_id JOIN COUNTRIES ON countries.country_id = locations.country_id ;
 
 -- 2 Write a query to find the name (first_name, last name), department ID and name of all the employees.

SELECT concat(FIRST_NAME,'  ',LAST_NAME) AS NAME,DEPARTMENTS.DEPARTMENT_ID,DEPARTMENT_NAME FROM DEPARTMENTS JOIN EMPLOYEES ON
DEPARTMENTS.DEPARTMENT_ID = EMPLOYEES.DEPARTMENT_ID; 

-- 3 Write a query to find the name (first_name, last_name), job, department ID and name of the employees who works in London.

SELECT concat(FIRST_NAME,'  ',LAST_NAME) AS NAME, JOB_TITLE , DEPARTMENTs.department_ID,DEPARTMENT_NAME,locations.city
...
SELECT  EMP.employee_id as Employee_ID, emp.last_name as Name, mgr.employee_id as Manager_id, mgr.last_name as Manager_Name
FROM employees as emp join employees as mgr on  mgr.employee_id = emp.manager_id;



-- 5 Write a query to find the name (first_name, last_name) and hire date of the employees who was hired after 'Jones.
select concat(A.first_name,'    ', A.last_name) as EMPLOYEE_NMAE, A.hire_date
from employees B JOIN employees A ON
 A.hire_date > B.hire_date and B.last_name = 'Jones';
...
select employees.employee_id,employees.department_id, job_title, datediff(end_date , start_date) 
 from job_history JOIN employees ON 
 employees.EMPLOYEE_ID= job_history.EMPLOYEE_ID join
 jobs ON employees.job_id = jobs.job_id and  
employees.DEPARTMENT_ID=90;

-- 8 Write a query to display the department ID and name and first name of manager. 
select employees.employee_ID, employees.department_id, department_name, first_name AS MANAGER_NAME
from employees JOIN departments ON
...
from employees JOIN departments ON 
employees.department_id = departments.department_id JOIN  locations ON
...
as diffsal from employees JOIN jobs ON
employees.job_id = jobs.job_id;
